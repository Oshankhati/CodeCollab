...
//       presenceMap.get(workspaceId).set(socket.id, { socketId: socket.id, user, fileId: null });
//       io.to(workspaceId).emit("presence-update", Array.from(presenceMap.get(workspaceId).values()));
//     });
...
//       // also join workspace if not already
//       if (workspaceId) {
//         socket.join(workspaceId);
//         if (!presenceMap.has(workspaceId)) presenceMap.set(workspaceId, new Map());
//         const users = presenceMap.get(workspaceId);
//         if (!users.has(socket.id)) users.set(socket.id, { socketId: socket.id, user, fileId });
//         else users.get(socket.id).fileId = fileId;
//         io.to(workspaceId).emit("presence-update", Array.from(users.values()));
//       }
//     });
...
//         const users = presenceMap.get(workspaceId);
//         if (users.has(socket.id)) users.get(socket.id).fileId = null;
//         io.to(workspaceId).emit("presence-update", Array.from(users.values()));
//       }
//     });
...
//       if (fileId && update) socket.to(fileId).emit("code-update", update);
//     });

//     /* typing */
//     socket.on("typing", ({ fileId, user }) => {
//       if (fileId && user) socket.to(fileId).emit("typing", user);
//     });

//     /* cursor */
//     socket.on("cursor-update", ({ fileId, user, position }) => {
//       if (fileId) socket.to(fileId).emit("cursor-update", { user, position });
//     });

//     /* file lock */
//     socket.on("file-lock", ({ fileId, user }) => {
//       if (fileId) io.to(fileId).emit("file-lock-update", { fileId, lockedBy: user });
//     });

//     /* file unlock */
//     socket.on("file-unlock", ({ fileId }) => {
//       if (fileId) io.to(fileId).emit("file-lock-update", { fileId, lockedBy: null });
//     });

//     /* ─────────────────────────────────────────
//        CHAT MESSAGE
//        Broadcasts to everyone in the workspace
//     ───────────────────────────────────────── */
//     socket.on("chat-message", ({ workspaceId, user, text }) => {
...
//       // broadcast to EVERYONE in workspace room including sender
//       io.to(workspaceId).emit("chat-message", msg);
//     });
...
//           io.to(wid).emit("presence-update", Array.from(users.values()));
//           if (users.size === 0) presenceMap.delete(wid);
//         }
//       }
//     });
...
// /* broadcast activity from controllers */
// export const emitActivity = (workspaceId, activity) => {
//   if (io) io.to(workspaceId).emit("workspace-activity", activity);
// };
